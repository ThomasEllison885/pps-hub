#!/usr/bin/env python3
"""Render cron entrypoint — POST to hub daily Estimates assignment/reminder API."""
...
READ_TIMEOUT = int(os.environ.get('ESTIMATE_CHECK_TIMEOUT', '300'))
WAKE_TIMEOUT = int(os.environ.get('ESTIMATE_CHECK_WAKE_TIMEOUT', '90'))
...
def _wake_hub(base):
    ...
    try:
        ...
        print(f'Wake ping: {e} (will still try estimate check)')
        return False
...
def main():
    ...
    url = base + '/api/cron/daily-estimate-check'
    ...
    try:
        ...
        if data.get('ok'):
            print(
                f"OK: checked {data.get('checked', '?')} estimates, "
                f"{data.get('new_assignments', 0)} new assignments, "
                f"{data.get('assignment_emails_sent', 0)} assignment emails, "
                f"{data.get('reminder_emails_sent', 0)} reminder emails"
            )
            return 0
        print('ERROR: estimate check endpoint returned ok=false')
        return 1
