#!/usr/bin/env python3
"""Render cron entrypoint — POST to hub weekly Trade Partner compliance API."""
...
READ_TIMEOUT = int(os.environ.get('TP_COMPLIANCE_TIMEOUT', '300'))
WAKE_TIMEOUT = int(os.environ.get('TP_COMPLIANCE_WAKE_TIMEOUT', '90'))
...
def _wake_hub(base):
    ...
    try:
        ...
        print(f'Wake ping: {e} (will still try compliance check)')
        return False
...
def main():
    ...
    url = base + '/api/cron/weekly-tp-compliance'
    ...
    try:
        ...
        if data.get('ok'):
            print(f"OK: checked {data.get('checked', '?')} subs, sent={data.get('sent')}")
            return 0
        print('ERROR: compliance endpoint returned ok=false')
        return 1
